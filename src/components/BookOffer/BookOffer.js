...
const BookOffer = () => {
  ...
  useEffect(() => {
    ...
    window.scrollTo(0, 0);
    fetch(`https://fast-gorge-50252.herokuapp.com/offers/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setData(data);
        setLoading(false);
        ...
        <>
          <div className="tour-details">
            ...
            <div className="d-flex justify-content-between">
              <p className="card-text duration">
                {" "}
                <i className="fas fa-clock"></i> {data.days} DAYS -{" "}
                {data.nights} NIGHTS{" "}
              </p>
          ...
          <div className="row gy-5">
            <div className="col-md-7">
              <img src={data.detailsImage} className="w-100 book-img" alt="" />
              <div className="details-div mt-3">
                <p className="details">Details</p>
                <hr />
                <p className="book-details">{data.details}</p>
              </div>
            </div>
